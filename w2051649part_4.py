...
Progression_OutcomeDictionary={}#Create a main Dictionary
...
while Prompt != "q":#This should be false to exit loop
    ...
    if passCredits == 120:
        ...
        print(ProgressionOutcome)
    print("")
    CreditsDictionary={'OutcomeResult':'text','Pass_Credits':'0','Defer_Credits':'0','Fail_Credits':'0'}#Create Dictionay inside a dictionary
    CreditsDictionary['OutcomeResult']=ProgressionOutcome
    CreditsDictionary['Pass_Credits']=passCredits
    CreditsDictionary['Defer_Credits']=deferCredits
    CreditsDictionary['Fail_Credits']=failCredits
    Progression_OutcomeDictionary[str(LoopCount)]=CreditsDictionary#Put sub dictionary into main dictionary
    while True:
...
FinalAnwser=Progress+Trailer+Retriver+Exclude#Calculate Outcome total
print(Progression_OutcomeDictionary)
print('''
part 4:''')#display items inside the dictionary
NumberOfItems=len(Progression_OutcomeDictionary)#find number of items in main dictionary
for number in range(NumberOfItems):
    key=number+1#find value using key
    ReCallSubDictionary=Progression_OutcomeDictionary[f"{key}"]
    print(ReCallSubDictionary['OutcomeResult']," - ",ReCallSubDictionary['Pass_Credits'],", ",ReCallSubDictionary['Defer_Credits'],", ",ReCallSubDictionary['Fail_Credits'],sep='')
